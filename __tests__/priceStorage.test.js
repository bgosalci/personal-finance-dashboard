...
test('notifies handlers on save', () => {
  ...
  const util = fs.readFileSync(path.resolve(__dirname, '../app/js/core/storageUtils.js'), 'utf8');
  vm.runInContext(util, context);
  const code = fs.readFileSync(path.resolve(__dirname, '../app/js/data/priceStorage.js'), 'utf8');
